...
class Serializer{
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        RealizeTemplateA a=new RealizeTemplateA(666);
        File file=new File("d:\\dev\\files\\objects.obj");
    ObjectOutputStream ous=new ObjectOutputStream(new FileOutputStream(file));
    ous.writeObject(a);
    ous.close();
    ObjectInputStream ois=new ObjectInputStream(new FileInputStream(file));
    RealizeTemplateA b= (RealizeTemplateA) ois.readObject();
        System.out.println(b.toString());

    }
}
abstract class Template implements Serializable{
    private static int st=1;
    private transient int  din;
    public Template(int din){this.din=din;}
    abstract void  foo();
    public String toString(){
        StringBuilder sb=new StringBuilder();

        return sb.append("st=").append(new Integer(st).toString()).append("  din=").append(new Integer(din)).toString();
    }
}
class RealizeTemplateA extends Template{


    public RealizeTemplateA(int din) {
        super(din);
    }
